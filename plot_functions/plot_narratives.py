...
def plot_narratives(x_data, ds_stacked, representative_groups, labels, cluster_names,
    ...
    for row_idx, row in enumerate(rows):
        for col_idx in range(len(cols)):
            ...
            ax.hlines(y=0, xmin=min_val, xmax=max_val, color='k', linestyle='--', linewidth=1, alpha=0.5, zorder=0)
            ax.vlines(x=0, ymin=min_val, ymax=max_val,color='k', linestyle='--', linewidth=1, alpha=0.5, zorder=0)
